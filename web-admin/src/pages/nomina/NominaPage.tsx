...
import {
  ...
  Settings, ListChecks, CalendarCog, Trash2, Pencil, Printer, Send,
} from 'lucide-react';
import {
  getEmpleados, crearEmpleado, actualizarEmpleado,
  getRoles, crearRol, actualizarRol, aprobarRol, marcarPagadoRol, enviarRolEmail,
...
function RolCard({ rol, onAprobar, onPagar, onEdit, onEnviar }: { rol: RolPago; onAprobar: () => void; onPagar: () => void; onEdit: () => void; onEnviar: () => void }) {
  const [open, setOpen] = useState(false);
  const ingresos = (rol.detalles || []).filter(d => d.tipo === 'INGRESO');
  const descuentos = (rol.detalles || []).filter(d => d.tipo === 'DESCUENTO');
  const totalProvisiones = num(rol.aporte_patronal) + num(rol.decimo_tercero) + num(rol.decimo_cuarto) + num(rol.fondos_reserva) + num(rol.vacaciones);
  const mostrarProvisiones = rol.empleado_afiliado_iess && totalProvisiones > 0;
  const printId = `rol-pago-print-${rol.id}`;
  ...
  return <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
    <div className="flex items-center justify-between px-4 py-3 cursor-pointer hover:bg-gray-50" onClick={() => setOpen(o => !o)}>
      ...
      <div className="flex items-center gap-4"><div className="text-right"><p className="text-xs text-gray-500">Líquido</p><p className="font-bold text-gray-900">{fmt(rol.liquido_a_pagar)}</p></div><span className={`text-xs px-2 py-1 rounded-full font-medium ${ESTADO_BADGE[rol.estado]}`}>{rol.estado}</span>
        <div className="flex gap-1" onClick={e => e.stopPropagation()}><button onClick={() => printElement(printId)} className="text-xs text-gray-600 border border-gray-300 rounded px-2 py-1 hover:bg-gray-50" title="Imprimir rol"><Printer size={11} className="inline" /> Imprimir</button><button onClick={onEnviar} className="text-xs text-teal-600 border border-teal-300 rounded px-2 py-1 hover:bg-teal-50" title="Enviar rol por email"><Send size={11} className="inline" /> Enviar</button>{rol.estado === 'BORRADOR' && <><button onClick={onEdit} className="text-xs text-gray-600 border border-gray-300 rounded px-2 py-1 hover:bg-gray-50">Editar</button><button onClick={onAprobar} className="text-xs text-blue-600 border border-blue-300 rounded px-2 py-1 hover:bg-blue-50">Aprobar</button></>}{rol.estado === 'APROBADO' && <button onClick={onPagar} className="text-xs text-green-600 border border-green-300 rounded px-2 py-1 hover:bg-green-50"><DollarSign size={11} className="inline" /> Pagar</button>}</div>
      </div>
    </div>
    <div id={printId} className="hidden">
      <div className="space-y-5 text-sm text-gray-800">
        <div className="border-b pb-4"><h1 className="text-xl font-bold text-gray-900">Rol de pago</h1><p>{MESES_LABEL[rol.mes]} {rol.anio}</p></div>
        <div className="grid grid-cols-2 gap-3"><div><p className="text-xs text-gray-500">Empleado</p><p className="font-semibold">{rol.empleado_nombre}</p></div><div><p className="text-xs text-gray-500">Estado</p><p className="font-semibold">{rol.estado}</p></div></div>
        <div className="grid grid-cols-2 gap-6"><div><h2 className="font-semibold text-green-700 mb-2">Ingresos</h2>{ingresos.length > 0 ? ingresos.map((d, i) => <Row key={d.id || i} label={d.descripcion || d.rubro_nombre || 'Ingreso'} val={d.valor_total || 0} />) : <Row label="Sueldo base" val={rol.sueldo_base} />}<div className="border-t mt-2 pt-2 flex justify-between font-bold"><span>Total ingresos</span><span>{fmt(rol.total_ingresos)}</span></div></div><div><h2 className="font-semibold text-red-700 mb-2">Descuentos</h2><Row label="Aporte personal IESS" val={rol.aporte_personal} />{descuentos.map((d, i) => <Row key={d.id || i} label={d.descripcion || d.rubro_nombre || 'Descuento'} val={d.valor_total || 0} />)}<div className="border-t mt-2 pt-2 flex justify-between font-bold"><span>Total descuentos</span><span>{fmt(rol.total_descuentos)}</span></div></div></div>
        <div className="border-t pt-4 flex justify-between text-lg font-bold"><span>Líquido a pagar</span><span>{fmt(rol.liquido_a_pagar)}</span></div>
        {rol.notas && <div><p className="text-xs text-gray-500">Notas</p><p>{rol.notas}</p></div>}
      </div>
    </div>
...
function RolModal({ empleados, rubros, rol, anio, mes, onClose, onSaved }: { empleados: Empleado[]; rubros: RubroNomina[]; rol?: RolPago; anio: number; mes: number; onClose: () => void; onSaved: () => void }) {
  ...
  const [detalles, setDetalles] = useState<DetalleRolPago[]>(() => {
    if (rol?.detalles?.length) return rol.detalles.map(d => {
      const rubro = rubros.find(r => r.id === Number(d.rubro));
      return { ...d, tipo: d.tipo || rubro?.tipo, cantidad: num(d.cantidad), valor_unitario: num(d.valor_unitario), valor_total: num(d.valor_total), orden: d.orden || rubro?.orden };
    });
    const sueldo = rubros.find(r => r.codigo === 'SUELDO_BASE') || rubros.find(r => r.tipo === 'INGRESO');
    return sueldo ? [{ rubro: sueldo.id, descripcion: sueldo.nombre, tipo: sueldo.tipo, cantidad: 1, valor_unitario: 0, orden: sueldo.orden }] : [];
  });
  const [saving, setSaving] = useState(false);

  const tipoDetalle = (detalle: DetalleRolPago) => detalle.tipo || rubroById.get(Number(detalle.rubro))?.tipo || 'INGRESO';
  const totalDetalle = (detalle: DetalleRolPago) => num(detalle.cantidad) * num(detalle.valor_unitario);
  ...
  const addDetalle = (tipo?: TipoRubroNomina) => {
    ...
    if (!rubro) return;
    setDetalles(rows => [...rows, { rubro: rubro.id, descripcion: rubro.nombre, tipo: rubro.tipo, cantidad: 1, valor_unitario: 0, orden: rubro.orden }]);
  };
  ...
  const handleRubroChange = (index: number, rubroId: number) => {
    ...
    setDetalle(index, { rubro: rubroId, descripcion: rubro?.nombre || '', tipo: rubro?.tipo, orden: rubro?.orden || 100 });
  };
  ...
  const handleEmpleadoChange = (value: string) => {
    ...
    setDetalles(rows => rows.length ? rows.map((row, index) => index === 0 ? { ...row, rubro: sueldo.id, descripcion: sueldo.nombre, tipo: sueldo.tipo, valor_unitario: emp.sueldo_base, cantidad: 1, orden: sueldo.orden } : row) : [{ rubro: sueldo.id, descripcion: sueldo.nombre, tipo: sueldo.tipo, cantidad: 1, valor_unitario: emp.sueldo_base, orden: sueldo.orden }]);
  };

  const totalIngresosPreview = detalles.filter(d => tipoDetalle(d) === 'INGRESO').reduce((acc, d) => acc + totalDetalle(d), 0);
  const totalDescuentosPreview = detalles.filter(d => tipoDetalle(d) === 'DESCUENTO').reduce((acc, d) => acc + totalDetalle(d), 0);
  const liquidoPreview = totalIngresosPreview - totalDescuentosPreview;
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const payload = { empleado: Number(empleado), anio, mes, notas, detalles: detalles.map((d, i) => ({ rubro: Number(d.rubro), descripcion: d.descripcion, cantidad: num(d.cantidad), valor_unitario: num(d.valor_unitario), orden: d.orden || rubroById.get(Number(d.rubro))?.orden || i + 1 })) };
  ...
  return <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4"><div className="bg-white rounded-xl shadow-xl w-full max-w-5xl max-h-[90vh] flex flex-col">
    ...
    <div className="overflow-y-auto flex-1 p-5 space-y-4">
      ...
      <div className="overflow-x-auto border border-gray-200 rounded-lg"><table className="w-full text-sm"><thead><tr className="bg-gray-50 text-xs text-gray-500"><th className="text-left px-3 py-2">Rubro</th><th className="text-left px-3 py-2 w-24">Tipo</th><th className="text-left px-3 py-2">Descripción</th><th className="text-right px-3 py-2 w-24">Cantidad</th><th className="text-right px-3 py-2 w-32">Valor</th><th className="text-right px-3 py-2 w-32">Total</th><th className="w-10"></th></tr></thead><tbody>{detalles.map((d, index) => { const tipo = tipoDetalle(d); return <tr key={index} className="border-t"><td className="px-3 py-2"><select value={d.rubro || ''} onChange={e => handleRubroChange(index, Number(e.target.value))} className="w-full border border-gray-300 rounded px-2 py-1 text-xs">{activeRubros.map(r => <option key={r.id} value={r.id}>{r.nombre}</option>)}</select></td><td className="px-3 py-2"><span className={`text-[11px] px-2 py-1 rounded-full ${TIPO_BADGE[tipo]}`}>{tipo}</span></td><td className="px-3 py-2"><input value={d.descripcion} onChange={e => setDetalle(index, { descripcion: e.target.value })} className="w-full border border-gray-300 rounded px-2 py-1 text-xs" /></td><td className="px-3 py-2"><input type="number" step="0.01" min="0" value={d.cantidad} onChange={e => setDetalle(index, { cantidad: num(e.target.value) })} className="w-full border border-gray-300 rounded px-2 py-1 text-xs text-right" /></td><td className="px-3 py-2"><input type="number" step="0.01" min="0" value={d.valor_unitario} onChange={e => setDetalle(index, { valor_unitario: num(e.target.value) })} className="w-full border border-gray-300 rounded px-2 py-1 text-xs text-right" /></td><td className={`px-3 py-2 text-right font-mono ${tipo === 'DESCUENTO' ? 'text-red-600' : 'text-gray-800'}`}>{tipo === 'DESCUENTO' ? '-' : ''}{fmt(totalDetalle(d))}</td><td className="px-2 py-2"><button onClick={() => removeDetalle(index)} className="p-1 text-gray-400 hover:text-red-600"><Trash2 size={14} /></button></td></tr>; })}</tbody></table></div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm"><div className="bg-green-50 border border-green-100 rounded-lg p-3"><span className="block text-xs text-green-700">Ingresos</span><span className="font-semibold font-mono text-green-800">{fmt(totalIngresosPreview)}</span></div><div className="bg-red-50 border border-red-100 rounded-lg p-3"><span className="block text-xs text-red-700">Descuentos</span><span className="font-semibold font-mono text-red-800">-{fmt(totalDescuentosPreview)}</span></div><div className="bg-gray-50 border border-gray-100 rounded-lg p-3"><span className="block text-xs text-gray-500">Líquido preliminar</span><span className="font-semibold font-mono text-gray-900">{fmt(liquidoPreview)}</span></div></div>
...
function PagarRolModal({ rol, cuentas, onClose, onSaved }: { rol: RolPago; cuentas: CuentaBancaria[]; onClose: () => void; onSaved: () => void }) {
  ...
  return <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4"><div className="bg-white rounded-xl shadow-xl w-full max-w-md"><div className="p-5 border-b"><h2 className="font-semibold text-gray-800">Pagar rol</h2><p className="text-sm text-gray-500">{rol.empleado_nombre} - {fmt(rol.liquido_a_pagar)}</p></div><div className="p-5 space-y-3"><div><label className="block text-xs font-medium text-gray-600 mb-1">Cuenta bancaria</label><select value={cuenta} onChange={e => setCuenta(e.target.value)} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm"><option value="">Sin movimiento bancario</option>{cuentasArray.filter(c => c.activa).map(c => <option key={c.id} value={c.id}>{c.banco} - {c.numero_cuenta}</option>)}</select></div><div><label className="block text-xs font-medium text-gray-600 mb-1">Fecha pago</label><input type="date" value={fecha} onChange={e => setFecha(e.target.value)} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" /></div><div><label className="block text-xs font-medium text-gray-600 mb-1">Referencia</label><input value={referencia} onChange={e => setReferencia(e.target.value)} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" /></div></div><div className="p-5 border-t flex justify-end gap-3"><button onClick={onClose} className="px-4 py-2 text-sm text-gray-600 border border-gray-300 rounded-lg">Cancelar</button><button onClick={handleSave} disabled={saving} className="px-4 py-2 text-sm text-white bg-green-600 rounded-lg disabled:opacity-50">{saving ? 'Guardando...' : 'Registrar pago'}</button></div></div></div>;
}

function RubroModal({ rubro, onClose, onSaved }: { rubro?: RubroNomina; onClose: () => void; onSaved: () => void }) {
  const { showToast } = useToast();
  const isEdit = !!rubro?.id;
  const [form, setForm] = useState({
    codigo: rubro?.codigo || '', nombre: rubro?.nombre || '', tipo: (rubro?.tipo || 'INGRESO') as TipoRubroNomina,
    aplica_iess: rubro?.aplica_iess ?? false, aplica_ir: rubro?.aplica_ir ?? false, es_recurrente: rubro?.es_recurrente ?? true,
    activo: rubro?.activo ?? true, orden: String(rubro?.orden ?? 100),
  });
  const [saving, setSaving] = useState(false);
  const save = async () => {
    if (!form.codigo || !form.nombre) { showToast('Código y nombre son requeridos.', 'error'); return; }
    setSaving(true);
    try {
      const payload = { ...form, orden: Number(form.orden || 100) };
      if (isEdit) await actualizarRubro(rubro!.id, payload); else await crearRubro(payload);
      showToast(isEdit ? 'Rubro actualizado.' : 'Rubro creado.', 'success');
      onSaved();
    } catch { showToast('No se pudo guardar el rubro.', 'error'); } finally { setSaving(false); }
  };
  return <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4"><div className="bg-white rounded-xl shadow-xl w-full max-w-xl">
    <div className="flex items-center justify-between p-5 border-b"><h2 className="text-lg font-semibold text-gray-800">{isEdit ? 'Editar rubro' : 'Nuevo rubro'}</h2><button onClick={onClose} className="text-gray-400 hover:text-gray-600 text-xl">&times;</button></div>
    <div className="p-5 grid grid-cols-1 md:grid-cols-2 gap-4"><div><label className="block text-xs font-medium text-gray-600 mb-1">Código *</label><input value={form.codigo} onChange={e => setForm(f => ({ ...f, codigo: e.target.value.toUpperCase() }))} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" /></div><div><label className="block text-xs font-medium text-gray-600 mb-1">Nombre *</label><input value={form.nombre} onChange={e => setForm(f => ({ ...f, nombre: e.target.value }))} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" /></div><div><label className="block text-xs font-medium text-gray-600 mb-1">Tipo</label><select value={form.tipo} onChange={e => setForm(f => ({ ...f, tipo: e.target.value as TipoRubroNomina }))} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm"><option value="INGRESO">Ingreso</option><option value="DESCUENTO">Descuento</option><option value="PROVISION">Provisión</option></select></div><div><label className="block text-xs font-medium text-gray-600 mb-1">Orden</label><input type="number" value={form.orden} onChange={e => setForm(f => ({ ...f, orden: e.target.value }))} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" /></div><label className="flex items-center gap-2 text-sm text-gray-700"><input type="checkbox" checked={form.aplica_iess} onChange={e => setForm(f => ({ ...f, aplica_iess: e.target.checked }))} /> Aplica IESS</label><label className="flex items-center gap-2 text-sm text-gray-700"><input type="checkbox" checked={form.aplica_ir} onChange={e => setForm(f => ({ ...f, aplica_ir: e.target.checked }))} /> Aplica IR</label><label className="flex items-center gap-2 text-sm text-gray-700"><input type="checkbox" checked={form.es_recurrente} onChange={e => setForm(f => ({ ...f, es_recurrente: e.target.checked }))} /> Permitir recurrente</label><label className="flex items-center gap-2 text-sm text-gray-700"><input type="checkbox" checked={form.activo} onChange={e => setForm(f => ({ ...f, activo: e.target.checked }))} /> Activo</label></div>
    <div className="p-5 border-t flex justify-end gap-3"><button onClick={onClose} className="px-4 py-2 text-sm text-gray-600 border border-gray-300 rounded-lg">Cancelar</button><button onClick={save} disabled={saving} className="px-4 py-2 text-sm text-white bg-indigo-600 rounded-lg disabled:opacity-50">{saving ? 'Guardando...' : 'Guardar'}</button></div>
  </div></div>;
}

function RubrosPanel({ rubros, onReload }: { rubros: RubroNomina[]; onReload: () => void }) {
  const { showToast } = useToast();
  const [editing, setEditing] = useState<RubroNomina | undefined>();
  const [showModal, setShowModal] = useState(false);
  const seed = async () => { await sembrarRubrosBase(); showToast('Rubros base disponibles.', 'success'); onReload(); };
  return <div className="space-y-4"><div className="flex justify-between gap-2 flex-wrap"><p className="text-sm text-gray-500">Catálogo de ingresos, descuentos y provisiones.</p><div className="flex gap-2"><button onClick={seed} className="px-3 py-2 border border-gray-300 rounded-lg text-sm">Crear rubros base</button><button onClick={() => { setEditing(undefined); setShowModal(true); }} className="flex items-center gap-2 px-4 py-2 text-sm text-white bg-indigo-600 rounded-lg"><Plus size={14} /> Nuevo rubro</button></div></div><div className="bg-white border border-gray-100 rounded-xl overflow-hidden"><table className="w-full text-sm"><thead><tr className="bg-gray-50 text-xs text-gray-500"><th className="text-left px-4 py-2">Código</th><th className="text-left px-4 py-2">Nombre</th><th className="text-left px-4 py-2">Tipo</th><th className="text-center px-4 py-2">IESS</th><th className="text-center px-4 py-2">Recurrente</th><th className="text-center px-4 py-2">Activo</th><th className="px-4 py-2"></th></tr></thead><tbody>{rubros.map(r => <tr key={r.id} className="border-t"><td className="px-4 py-2 font-mono text-xs">{r.codigo}</td><td className="px-4 py-2">{r.nombre}</td><td className="px-4 py-2"><span className={`text-xs px-2 py-1 rounded-full ${TIPO_BADGE[r.tipo]}`}>{r.tipo}</span></td><td className="px-4 py-2 text-center">{r.aplica_iess ? 'Sí' : 'No'}</td><td className="px-4 py-2 text-center">{r.es_recurrente ? 'Sí' : 'No'}</td><td className="px-4 py-2 text-center"><button onClick={async () => { await actualizarRubro(r.id, { activo: !r.activo }); onReload(); }} className="text-xs text-indigo-600">{r.activo ? 'Activo' : 'Inactivo'}</button></td><td className="px-4 py-2 text-right"><button onClick={() => { setEditing(r); setShowModal(true); }} className="inline-flex items-center gap-1 text-xs text-indigo-600"><Pencil size={12} /> Editar</button></td></tr>)}</tbody></table></div>{showModal && <RubroModal rubro={editing} onClose={() => setShowModal(false)} onSaved={() => { setShowModal(false); onReload(); }} />}</div>;
}

function ConceptoModal({ empleados, rubros, concepto, onClose, onSaved }: { empleados: Empleado[]; rubros: RubroNomina[]; concepto?: ConceptoEmpleadoNomina; onClose: () => void; onSaved: () => void }) {
  const { showToast } = useToast();
  const isEdit = !!concepto?.id;
  const recurrentes = rubros.filter(r => r.activo && r.es_recurrente && r.codigo !== 'SUELDO_BASE');
  const [form, setForm] = useState({
    empleado: String(concepto?.empleado || ''), rubro: String(concepto?.rubro || ''), valor: String(concepto?.valor || ''),
    descripcion: concepto?.descripcion || '', fecha_inicio: concepto?.fecha_inicio || new Date().toISOString().slice(0, 10),
    permanente: concepto?.permanente ?? true, meses_duracion: concepto?.meses_duracion ? String(concepto.meses_duracion) : '',
    activo: concepto?.activo ?? true, notas: concepto?.notas || '',
  });
  const [saving, setSaving] = useState(false);
  const save = async () => {
    if (!form.empleado || !form.rubro || !form.valor) { showToast('Empleado, rubro y valor son requeridos.', 'error'); return; }
    if (!form.permanente && !form.meses_duracion) { showToast('Indique cuántos meses dura la recurrencia.', 'error'); return; }
    setSaving(true);
    try {
      const payload = { empleado: Number(form.empleado), rubro: Number(form.rubro), valor: Number(form.valor), descripcion: form.descripcion, fecha_inicio: form.fecha_inicio || null, permanente: form.permanente, meses_duracion: form.permanente ? null : Number(form.meses_duracion), activo: form.activo, notas: form.notas };
      if (isEdit) await actualizarConceptoEmpleado(concepto!.id, payload); else await crearConceptoEmpleado(payload);
      showToast(isEdit ? 'Recurrente actualizado.' : 'Recurrente creado.', 'success');
      onSaved();
    } catch { showToast('No se pudo guardar el recurrente.', 'error'); } finally { setSaving(false); }
  };
  return <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4"><div className="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col"><div className="flex items-center justify-between p-5 border-b"><h2 className="text-lg font-semibold text-gray-800">{isEdit ? 'Editar recurrente' : 'Nuevo recurrente'}</h2><button onClick={onClose} className="text-gray-400 hover:text-gray-600 text-xl">&times;</button></div><div className="overflow-y-auto p-5 grid grid-cols-1 md:grid-cols-2 gap-4"><div><label className="block text-xs font-medium text-gray-600 mb-1">Empleado *</label><select value={form.empleado} onChange={e => setForm(f => ({ ...f, empleado: e.target.value }))} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm"><option value="">Seleccione</option>{empleados.map(e => <option key={e.id} value={e.id}>{e.apellidos}, {e.nombres}</option>)}</select></div><div><label className="block text-xs font-medium text-gray-600 mb-1">Rubro *</label><select value={form.rubro} onChange={e => setForm(f => ({ ...f, rubro: e.target.value }))} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm"><option value="">Seleccione</option>{recurrentes.map(r => <option key={r.id} value={r.id}>{r.nombre} - {r.tipo}</option>)}</select></div><div><label className="block text-xs font-medium text-gray-600 mb-1">Valor *</label><input type="number" step="0.01" min="0" value={form.valor} onChange={e => setForm(f => ({ ...f, valor: e.target.value }))} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" /></div><div><label className="block text-xs font-medium text-gray-600 mb-1">Fecha inicio</label><input type="date" value={form.fecha_inicio} onChange={e => setForm(f => ({ ...f, fecha_inicio: e.target.value }))} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" /></div><div className="md:col-span-2"><label className="block text-xs font-medium text-gray-600 mb-1">Descripción</label><input value={form.descripcion} onChange={e => setForm(f => ({ ...f, descripcion: e.target.value }))} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" /></div><label className="flex items-center gap-2 text-sm text-gray-700"><input type="checkbox" checked={form.permanente} onChange={e => setForm(f => ({ ...f, permanente: e.target.checked, meses_duracion: e.target.checked ? '' : f.meses_duracion }))} /> Permanente</label>{!form.permanente && <div><label className="block text-xs font-medium text-gray-600 mb-1">Meses de recurrencia *</label><input type="number" min="1" step="1" value={form.meses_duracion} onChange={e => setForm(f => ({ ...f, meses_duracion: e.target.value }))} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" /></div>}<label className="flex items-center gap-2 text-sm text-gray-700"><input type="checkbox" checked={form.activo} onChange={e => setForm(f => ({ ...f, activo: e.target.checked }))} /> Activo</label><div className="md:col-span-2"><label className="block text-xs font-medium text-gray-600 mb-1">Notas</label><textarea rows={2} value={form.notas} onChange={e => setForm(f => ({ ...f, notas: e.target.value }))} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" /></div></div><div className="p-5 border-t flex justify-end gap-3"><button onClick={onClose} className="px-4 py-2 text-sm text-gray-600 border border-gray-300 rounded-lg">Cancelar</button><button onClick={save} disabled={saving} className="px-4 py-2 text-sm text-white bg-indigo-600 rounded-lg disabled:opacity-50">{saving ? 'Guardando...' : 'Guardar'}</button></div></div></div>;
}
...
function ConceptosPanel({ empleados, rubros, conceptos, onReload }: { empleados: Empleado[]; rubros: RubroNomina[]; conceptos: ConceptoEmpleadoNomina[]; onReload: () => void }) {
  const [editing, setEditing] = useState<ConceptoEmpleadoNomina | undefined>();
  const [showModal, setShowModal] = useState(false);
  return <div className="space-y-4"><div className="flex justify-between gap-2 flex-wrap"><p className="text-sm text-gray-500">Rubros fijos por empleado, con recurrencia permanente o por meses.</p><button onClick={() => { setEditing(undefined); setShowModal(true); }} className="flex items-center gap-2 px-4 py-2 text-sm text-white bg-indigo-600 rounded-lg"><Plus size={14} /> Nuevo recurrente</button></div><div className="bg-white border border-gray-100 rounded-xl overflow-hidden"><table className="w-full text-sm"><thead><tr className="bg-gray-50 text-xs text-gray-500"><th className="text-left px-4 py-2">Empleado</th><th className="text-left px-4 py-2">Rubro</th><th className="text-left px-4 py-2">Recurrencia</th><th className="text-right px-4 py-2">Valor</th><th className="text-center px-4 py-2">Estado</th><th className="px-4 py-2"></th></tr></thead><tbody>{conceptos.map(c => <tr key={c.id} className="border-t"><td className="px-4 py-2">{c.empleado_nombre}</td><td className="px-4 py-2"><div>{c.descripcion || c.rubro_nombre}</div><span className={`text-[11px] px-2 py-0.5 rounded-full ${TIPO_BADGE[c.rubro_tipo]}`}>{c.rubro_tipo}</span></td><td className="px-4 py-2 text-gray-600">{c.permanente ? 'Permanente' : `${c.meses_duracion || 0} mes(es)`}{c.fecha_fin ? <span className="block text-xs text-gray-400">Hasta {c.fecha_fin}</span> : null}</td><td className="px-4 py-2 text-right font-mono">{fmt(c.valor)}</td><td className="px-4 py-2 text-center"><button onClick={async () => { await actualizarConceptoEmpleado(c.id, { activo: !c.activo }); onReload(); }} className="text-xs text-indigo-600">{c.activo ? 'Activo' : 'Inactivo'}</button></td><td className="px-4 py-2 text-right"><button onClick={() => { setEditing(c); setShowModal(true); }} className="inline-flex items-center gap-1 text-xs text-indigo-600"><Pencil size={12} /> Editar</button></td></tr>)}</tbody></table></div>{showModal && <ConceptoModal empleados={empleados} rubros={rubros} concepto={editing} onClose={() => setShowModal(false)} onSaved={() => { setShowModal(false); onReload(); }} />}</div>;
}

function ParametroModal({ parametro, onClose, onSaved }: { parametro?: ParametroNomina; onClose: () => void; onSaved: () => void }) {
  const { showToast } = useToast();
  const isEdit = !!parametro?.id;
  const year = new Date().getFullYear();
  const [form, setForm] = useState({ anio: String(parametro?.anio || year), sbu: String(parametro?.sbu || '460.00'), aporte_personal_iess: String(parametro?.aporte_personal_iess || '0.0945'), aporte_patronal_iess: String(parametro?.aporte_patronal_iess || '0.1215'), decimo_tercero_factor: String(parametro?.decimo_tercero_factor || '0.083333'), vacaciones_factor: String(parametro?.vacaciones_factor || '0.041667'), fondo_reserva_factor: String(parametro?.fondo_reserva_factor || '0.083333'), activo: parametro?.activo ?? true, notas: parametro?.notas || '' });
  const [saving, setSaving] = useState(false);
  const save = async () => { setSaving(true); try { const payload = { anio: Number(form.anio), sbu: Number(form.sbu), aporte_personal_iess: Number(form.aporte_personal_iess), aporte_patronal_iess: Number(form.aporte_patronal_iess), decimo_tercero_factor: Number(form.decimo_tercero_factor), vacaciones_factor: Number(form.vacaciones_factor), fondo_reserva_factor: Number(form.fondo_reserva_factor), activo: form.activo, notas: form.notas }; if (isEdit) await actualizarParametroNomina(parametro!.id, payload); else await crearParametroNomina(payload); showToast(isEdit ? 'Parámetro actualizado.' : 'Parámetro creado.', 'success'); onSaved(); } catch { showToast('No se pudo guardar el parámetro.', 'error'); } finally { setSaving(false); } };
  const input = (label: string, key: keyof typeof form, step = '0.0001') => <div><label className="block text-xs font-medium text-gray-600 mb-1">{label}</label><input type="number" step={step} value={String(form[key])} onChange={e => setForm(f => ({ ...f, [key]: e.target.value }))} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" /></div>;
  return <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4"><div className="bg-white rounded-xl shadow-xl w-full max-w-2xl"><div className="flex items-center justify-between p-5 border-b"><h2 className="text-lg font-semibold text-gray-800">{isEdit ? 'Editar parámetro' : 'Nuevo parámetro'}</h2><button onClick={onClose} className="text-gray-400 hover:text-gray-600 text-xl">&times;</button></div><div className="p-5 grid grid-cols-1 md:grid-cols-2 gap-4">{input('Año', 'anio', '1')}{input('SBU', 'sbu', '0.01')}{input('IESS personal', 'aporte_personal_iess')}{input('IESS patronal', 'aporte_patronal_iess')}{input('Décimo tercero factor', 'decimo_tercero_factor')}{input('Vacaciones factor', 'vacaciones_factor')}{input('Fondo reserva factor', 'fondo_reserva_factor')}<label className="flex items-center gap-2 text-sm text-gray-700"><input type="checkbox" checked={form.activo} onChange={e => setForm(f => ({ ...f, activo: e.target.checked }))} /> Activo</label><div className="md:col-span-2"><label className="block text-xs font-medium text-gray-600 mb-1">Notas</label><textarea rows={2} value={form.notas} onChange={e => setForm(f => ({ ...f, notas: e.target.value }))} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" /></div></div><div className="p-5 border-t flex justify-end gap-3"><button onClick={onClose} className="px-4 py-2 text-sm text-gray-600 border border-gray-300 rounded-lg">Cancelar</button><button onClick={save} disabled={saving} className="px-4 py-2 text-sm text-white bg-indigo-600 rounded-lg disabled:opacity-50">{saving ? 'Guardando...' : 'Guardar'}</button></div></div></div>;
}

function ParametrosPanel({ parametros, onReload }: { parametros: ParametroNomina[]; onReload: () => void }) {
  const [editing, setEditing] = useState<ParametroNomina | undefined>();
  const [showModal, setShowModal] = useState(false);
  return <div className="space-y-4"><div className="flex justify-between gap-2 flex-wrap"><p className="text-sm text-gray-500">Parámetros legales y factores usados en el cálculo de roles.</p><button onClick={() => { setEditing(undefined); setShowModal(true); }} className="flex items-center gap-2 px-4 py-2 text-sm text-white bg-indigo-600 rounded-lg"><Plus size={14} /> Nuevo parámetro</button></div><div className="bg-white border border-gray-100 rounded-xl overflow-hidden"><table className="w-full text-sm"><thead><tr className="bg-gray-50 text-xs text-gray-500"><th className="text-left px-4 py-2">Año</th><th className="text-right px-4 py-2">SBU</th><th className="text-right px-4 py-2">IESS personal</th><th className="text-right px-4 py-2">IESS patronal</th><th className="text-center px-4 py-2">Estado</th><th className="px-4 py-2"></th></tr></thead><tbody>{parametros.map(p => <tr key={p.id} className="border-t"><td className="px-4 py-2">{p.anio}</td><td className="px-4 py-2 text-right">{fmt(p.sbu)}</td><td className="px-4 py-2 text-right">{Number(p.aporte_personal_iess) * 100}%</td><td className="px-4 py-2 text-right">{Number(p.aporte_patronal_iess) * 100}%</td><td className="px-4 py-2 text-center"><button onClick={async () => { await actualizarParametroNomina(p.id, { activo: !p.activo }); onReload(); }} className="text-xs text-indigo-600">{p.activo ? 'Activo' : 'Inactivo'}</button></td><td className="px-4 py-2 text-right"><button onClick={() => { setEditing(p); setShowModal(true); }} className="inline-flex items-center gap-1 text-xs text-indigo-600"><Pencil size={12} /> Editar</button></td></tr>)}</tbody></table></div>{showModal && <ParametroModal parametro={editing} onClose={() => setShowModal(false)} onSaved={() => { setShowModal(false); onReload(); }} />}</div>;
}
...
export default function NominaPage() {
  ...
  const loadCuentas = useCallback(async () => { const data = await getCuentas(); setCuentas(Array.isArray(data) ? data : []); }, []);
  ...
  const handleEnviarRol = async (id: number) => { try { const res = await enviarRolEmail(id); showToast(res.detail || 'Rol enviado.', 'success'); } catch (err: unknown) { const e = err as { response?: { data?: { detail?: string } } }; showToast(e?.response?.data?.detail || 'No se pudo enviar el rol.', 'error'); } };
