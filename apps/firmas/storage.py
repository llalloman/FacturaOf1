...
@deconstructible
class SignatureDocumentStorage(Storage):
    """
    Storage para documentos sensibles de solicitudes de firma.

    Usa S3/R2 cuando SIGNATURE_DOCUMENTS_STORAGE=s3; en local mantiene
    FileSystemStorage para no romper desarrollo si faltan credenciales.
    """
    ...
    def _build_storage(self):
        backend = getattr(settings, 'SIGNATURE_DOCUMENTS_STORAGE', 'local')
        if backend != 's3':
        ...
        try:
            ...
            try:
                ...
                raise ImproperlyConfigured(
                    'SIGNATURE_DOCUMENTS_STORAGE=s3 requiere instalar django-storages y boto3.'
                ) from exc
        ...
        if missing:
            raise ImproperlyConfigured(
                'Faltan variables para documentos S3/R2: '
                + ', '.join(missing)
                + '. Revisa AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, '
                'AWS_STORAGE_BUCKET_NAME y AWS_S3_ENDPOINT_URL.'
            )
